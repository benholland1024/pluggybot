...
from the two that already work: the **pen** (an X-Y plotter, `tools/drawing.py`)
and the **claw** (a pick-and-place gripper, `tools/gripper.py`). Both were built
...
(`tools/dispenser.py`, the fifth tool) was built from this doc rather than mined
...
### 3. The controller class, in `src/pluggybot/<tool>.py`
...
`mission/mission.py` handles the navigation half. Two things a new tool inherits
...
[ ] controller class in src/pluggybot/<tool>.py, tolerance class stated
