...
- **`recorder.py`** — `FrameBuilder` (decimation to ~20 Hz of sim time +
  ...
  emitted) and `TelemetryRecorder` (v0: builder + a JSONL writer thread).
...
- **`publisher.py`** — `WsPublisher` (v1): an outbound WebSocket
  **client** (the sim owns no public surface — if the endpoint is down,
  the robot keeps living). Same `FrameBuilder` as the recorder, so live
  and recorded frames are identical. A bounded queue feeds a sender
...
recording of the same run. `ws_sink.py` measures received frame *gaps* —
the wall-clock spacing between frames — which is the consumer-side proof
the stream is smooth.

## Measured (dev machine, 2026-08-14)
