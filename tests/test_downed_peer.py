...
to the reckoner, MEASURED 0.5-2.2 m off the body in 10 s. So the disc went
with the belief, and nothing routed round the body.
...
from pluggybot.mission.mission import (
  CLOSE_ENOUGH_M, DOWN_ROBOT_CELLS, OTHER_ROBOT_CELLS, HubMission,
)
...
def test_a_robot_lying_down_is_avoided_where_it_lies_not_where_it_says(lie):
  ...
  x, y, cells = me.mission.others[0]()
  assert cells == DOWN_ROBOT_CELLS
...
def test_a_robot_lying_down_is_not_held_for():
  ...
  robot lying down (250-600 points of it, every orientation measured), and
  the hold that channel drives is for a robot that will MOVE. One on the
  floor will not, and a detour that runs straight at it and turns at its
  disc's edge was held 0.52 m short until the drive timed out."""
...
def test_a_goal_beside_a_robot_lying_down_is_out_of_reach_further_off():
  ...
  for standing, reachable in ((True, True), (False, False)):
    m.others = [(lambda: (gx + 0.5, gy)) if standing
                else (lambda: (gx + 0.5, gy, DOWN_ROBOT_CELLS))]
